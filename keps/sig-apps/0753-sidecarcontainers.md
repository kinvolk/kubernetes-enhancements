...
to add a `lifecycle.type` field to the `container` object in the `pod.spec` to define if a
...
The concept of sidecar containers has been around since the early days of Kubernetes. A clear example is [this Kubernetes blog post][sidecar-blog-post] from
...
before others or stop after others, in these examples we will use a service mesh, metrics
gathering and logging sidecar.
...
or over the network on localhost, etc.).
// I don't really understand this sentence
Startup is usually not that critical,
because in the case where the other container is not started (like a restart) is
important, it is usually handled correctly.
...
Service mesh presents a similar problem: you want the service mesh container to be running and ready before other containers start, so that any inbound/outbound connections that a container can
...
The limitations due to the lack of ordering guarantees on startup and shutdown, can
sometimes forces the coupling of application code with infrastructure. This causes a dependency between applications and infrastructure, and forces more
...
This KEP removes the need for a service mesh to use either an
...
here as an example to show thee possibility of coupling apps with infrastructure. Similar
examples also exist in-house for non-open source applications too.
...
hooks were delivered only once most of the time. The problem is not if this
is idempotent or not (as it should be in any case), but chaining the most common
case from delivering preStop hook once to twice.
...
issues and will probably need to patch Kubernetes because of this. This could also result in a large increase in calls to the Kubernetes API server causing scalability issues.
...
A side effect of this behaviour is that killing a pod with sidecar containers can
...
This behaviour is because in the PR implementation the `gracePeriodOverride`
...
The previous section is about how, at the implementation level, to respect a given
time when killing a pod. This section, in contrast, is about how to split the
...
This alternative is also weird in the following way: a bad neighbour non-sidecar
neighbour can make sidecars never execute. The goal of using sidecar containers
...
graceful shutdown doesn't seem like a good reason to do have this behaviour in
Kubernetes.
...
If this behaviour is not welcome, however, code probably can be adapted to handle
...
Furthermore, service meshes don't provide easy ways to use them with initContainers
...
changes to the pod phase is a behavioural change regarding [current documentation
...
several users (properly drain connections, reduce errors on shutdown or even
...
or independant team, a use case that is every time more common.
...
ordering:
 * Use an initContainer to add logic to blackhole all the traffic until service
   mesh container is up
...
<!--
One common library that causes this on rails can be this: https://github.com/redis-store/redis-rails
